...
**Invoked by:** [`java-code-review`] after all critical issues resolved; [`maven-dependency-update`] after successful dependency updates; [`adr`] when committing an ADR alongside related changes; [`design-snapshot`] to commit a snapshot file; [`idea-log`] to commit IDEAS.md additions; [`write-blog`] to commit a blog entry
