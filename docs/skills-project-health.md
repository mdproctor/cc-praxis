...
**This skill builds on [`project-health`](project-health.md).** When invoked directly (e.g. `/skills-project-health`), it runs all universal checks first then its own additions — identical output to `project-health` auto-chaining to this skill. Either entry point produces the same result.
