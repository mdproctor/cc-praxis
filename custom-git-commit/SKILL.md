...
**Invoked by:** User says "commit" in a type: custom project (after git-commit routes here); [`adr`] to commit a new ADR; [`design-snapshot`] to commit a snapshot file; [`idea-log`] to commit IDEAS.md additions; [`write-content`] to commit a blog entry
